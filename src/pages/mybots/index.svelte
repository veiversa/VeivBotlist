...
<script>
	...
	async function getBots() {
		// @ts-ignore
		const data = await fetch(`${import.meta.env.VITE_API_URL}/v1/users/${user.id}`).then((res) =>
			res.json()
		);

		return data.user.bots;
	}
</script>
...
<div class="flex w-full mt-2">
	...
	<div class="container oveflow-y-auto mt-10">
		<TextBlock variant="titleLarge" style="color:white;">Your Bots</TextBlock>

		{#await getBots()}
			<!--  -->
		{:then bots}<div class="grid grid-cols-3 gap-2 mt-6">
				{#if bots.length}
					{#each bots as bot}
						<div
							class="w-64 bg-[#fafafa] bg-opacity-5 rounded-lg border border-[#141f24] shadow-md hover:shadow-lg hover:bg-opacity-10 transition gap-7"
						>
							<img class="rounded-full mx-auto mt-4" src={bot.avatar_url} alt="" />

							<div class="p-4">
								<a href="#c">
									<h5 class="mb-1 text-2xl font-bold tracking-tight text-white">{bot.username}</h5>
								</a>
								<p class="mb-3 font-normal  text-gray-700">{bot.shortDescription}</p>
				...
				{:else}
					<p class="text-gray-400 text-2xl font-semibold">
						I'm sorry, there's nothing to show here.
					</p>
				{/if}
			</div>
		{/await}
